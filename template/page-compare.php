<?php
$product_list = $this->asex_get_product_compare();
$total_attrs  = array();
$attrs        = array();
$images       = array();
$names        = array();
...
if (!empty($product_list)) {
    ...
    foreach ($product_list as
        ...
        $views[$id]   = '<a target="_blank" href="' . get_permalink($id) . '">' . esc_attr__("View Product", 'asex') . '</a>';
        $removes[$id] = '<a class="asex_remove_compare_product_btn" data-product-id="' . $id . '" href="javascript:;">' . esc_html__("Remove", 'asex') . '</a>';
        $product_attr = $product["attributes"];
...
//|<a href="javascript:;" class="button asex_button product_show_detail_button asex_quickview_btn" data-ajax-url="' . admin_url("admin-ajax.php") . '" data-product-id="' . $id . '"><span class="dslc-icon dslc-icon-eye-open"></span></a>
?>
    ...
    <div class="as-table-list-compare">
        <table>
            ...
            if (empty($product_list)) {
                ?>
                <tr>
                    <td><span class="as-no-product-msg"><?php esc_attr_e("No Products in Compare List", 'asex'); ?></span></td>
                </tr>
                <?php
            }
            else {
                ?>
                <tr>
                    <td><?php esc_attr_e("Images", 'asex'); ?></td>
                    <?php
                    foreach ($images as
                            $id =>
                            $image) {
                ...
                <tr>
                    <td><?php esc_attr_e("Names", 'asex'); ?></td>
                    <?php
                    foreach ($names as
                            $id =>
                            $name) {
                ...
                <tr>
                    <td><?php esc_attr_e("Price", 'asex'); ?></td>
                    <?php
                    foreach ($prices as
                            $id =>
                            $price) {
                ...
                ?>
                <tr>
                    <td><?php esc_attr_e("View", 'asex'); ?></td>
                    <?php
                    foreach ($views as
                            $id =>
                            $view) {
                ...
                <tr>
                    <td><?php esc_attr_e("Remove", 'asex'); ?></td>
                    <?php
                    foreach ($removes as
                            $id =>
                            $remove) {
